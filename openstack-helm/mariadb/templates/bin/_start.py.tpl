...
# so symlinked it.
if not os.path.exists("/usr/lib/mysql/plugin/client_ed25519.so"):
...
# Heechul: get MYSQL_BINARY_NAME from env variable since it could be
# mariadbd.
if check_env_var("MYSQL_BINARY_NAME"):
    MYSQL_BINARY_NAME = os.environ['MYSQL_BINARY_NAME']
else:
    MYSQL_BINARY_NAME = "mysqld"
if check_env_var("MYSQL_BINARY_INSTALL_DB"):
    MYSQL_BINARY_INSTALL_DB = os.environ['MYSQL_BINARY_INSTALL_DB']
else:
    MYSQL_BINARY_INSTALL_DB = "mysql_install_db"
if check_env_var("MYSQL_BINARY_UPGRADE"):
    MYSQL_BINARY_UPGRADE = os.environ['MYSQL_BINARY_UPGRADE']
else:
    MYSQL_BINARY_UPGRADE = "mysql_upgrade"
...
def run_mysqld(cluster='existing'):
    ...
    if os.path.isdir(db_test_dir):
        ...
        upgrade_cmd=[MYSQL_BINARY_UPGRADE, '--skip-write-binlog',
